...
-- Step one ===================================================================
-- Color scheme is configured in 'plugin/40_plugins.lua' to use external theme.
...
now(function()
	...
	local ext_blocklist = { scm = true, txt = true, yml = true, json = true, yaml = true }
	require("mini.icons").setup({
		use_file_extension = function(ext, _)
			return not ext_blocklist[ext]
		end,
	})
...
now_if_args(function()
	...
	-- Patch: MiniMisc.find_root() caches root directories but never rechecks
	-- existence on cache hit. After deleting a worktree, the stale cached path
	-- causes vim.fn.chdir() to throw E344. Override the autocommand to guard
	-- against non-existent directories. Remove when fixed upstream.
	...
	vim.api.nvim_create_autocmd("BufEnter", {
		...
		nested = true,
		callback = set_root,
		desc = "Find root and change current directory (patched: skip deleted dirs)",
	})
...
later(function()
  ...
  miniclue.setup({
    ...
    -- Keep clue popups focused on workflow and navigation keys.
    triggers = {
...
-- MiniGit is intentionally disabled to let 'vim-fugitive' own `:Git` commands.
-- This keeps one Git command surface and avoids conflicts between providers.
...
later(function()
	...
	for _, key in ipairs({ "n", "N", "*", "#" }) do
		local rhs = key
			-- Center screen and open enough folds when jumping to the next match
			.. "zzzv"
