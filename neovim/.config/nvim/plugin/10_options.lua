...
vim.o.number         = false      -- Don't show absolute line numbers
...
vim.o.textwidth     = 100     -- Wrap at 100 columns by default
...
-- Keep `textwidth` at most 100, but tighter when a window is narrower.
local set_textwidth = function()
  local win = vim.api.nvim_get_current_win()
  local width = vim.api.nvim_win_get_width(win)
  local wininfo = vim.fn.getwininfo(win)[1] or {}
  local textoff = wininfo.textoff or 0
  local available = math.max(1, width - textoff)

  vim.bo.textwidth = math.min(100, available)
end
_G.Config.new_autocmd({ 'BufWinEnter', 'WinEnter', 'WinResized' }, nil, set_textwidth, 'Adapt textwidth to window')
...
local diagnostic_opts = {
  ...
  -- Update diagnostics in real-time while typing
